import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated tv_displays icon from Google Material Icons
class MconTvDisplays extends MconBase {
  const MconTvDisplays({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconTvDisplays> createState() => _MconTvDisplaysState();
}

class _MconTvDisplaysState extends MconBaseState<MconTvDisplays> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconTvDisplaysPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconTvDisplaysPainter extends MconPainter {
  _MconTvDisplaysPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(80.0), y(-320.0));
    path.lineTo(x(80.0), y(-800.0));
    path.quadraticBezierTo(x(80.0), y(-833.0), x(103.5), y(-856.5));
    path.quadraticBezierTo(x(127.0), y(-880.0), x(160.0), y(-880.0));
    path.lineTo(x(720.0), y(-880.0));
    path.lineTo(x(720.0), y(-800.0));
    path.lineTo(x(160.0), y(-800.0));
    path.lineTo(x(160.0), y(-320.0));
    path.lineTo(x(80.0), y(-320.0));
    path.close();
    path.moveTo(x(440.0), y(-80.0));
    path.lineTo(x(440.0), y(-160.0));
    path.lineTo(x(320.0), y(-160.0));
    path.quadraticBezierTo(x(287.0), y(-160.0), x(263.5), y(-183.5));
    path.quadraticBezierTo(x(240.0), y(-207.0), x(240.0), y(-240.0));
    path.lineTo(x(240.0), y(-640.0));
    path.quadraticBezierTo(x(240.0), y(-673.0), x(263.5), y(-696.5));
    path.quadraticBezierTo(x(287.0), y(-720.0), x(320.0), y(-720.0));
    path.lineTo(x(840.0), y(-720.0));
    path.quadraticBezierTo(x(873.0), y(-720.0), x(896.5), y(-696.5));
    path.quadraticBezierTo(x(920.0), y(-673.0), x(920.0), y(-640.0));
    path.lineTo(x(920.0), y(-240.0));
    path.quadraticBezierTo(x(920.0), y(-207.0), x(896.5), y(-183.5));
    path.quadraticBezierTo(x(873.0), y(-160.0), x(840.0), y(-160.0));
    path.lineTo(x(720.0), y(-160.0));
    path.lineTo(x(720.0), y(-80.0));
    path.lineTo(x(440.0), y(-80.0));
    path.close();
    path.moveTo(x(320.0), y(-240.0));
    path.lineTo(x(840.0), y(-240.0));
    path.lineTo(x(840.0), y(-640.0));
    path.lineTo(x(320.0), y(-640.0));
    path.lineTo(x(320.0), y(-240.0));
    path.close();
    path.moveTo(x(580.0), y(-440.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
