import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated pergola icon from Google Material Icons
class MconPergola extends MconBase {
  const MconPergola({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconPergola> createState() => _MconPergolaState();
}

class _MconPergolaState extends MconBaseState<MconPergola> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconPergolaPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconPergolaPainter extends MconPainter {
  _MconPergolaPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(120.0), y(-120.0));
    path.lineTo(x(120.0), y(-840.0));
    path.quadraticBezierTo(x(120.0), y(-857.0), x(131.5), y(-868.5));
    path.quadraticBezierTo(x(143.0), y(-880.0), x(160.0), y(-880.0));
    path.quadraticBezierTo(x(177.0), y(-880.0), x(188.5), y(-868.5));
    path.quadraticBezierTo(x(200.0), y(-857.0), x(200.0), y(-840.0));
    path.lineTo(x(200.0), y(-800.0));
    path.lineTo(x(760.0), y(-800.0));
    path.lineTo(x(760.0), y(-840.0));
    path.quadraticBezierTo(x(760.0), y(-857.0), x(771.5), y(-868.5));
    path.quadraticBezierTo(x(783.0), y(-880.0), x(800.0), y(-880.0));
    path.quadraticBezierTo(x(817.0), y(-880.0), x(828.5), y(-868.5));
    path.quadraticBezierTo(x(840.0), y(-857.0), x(840.0), y(-840.0));
    path.lineTo(x(840.0), y(-120.0));
    path.lineTo(x(760.0), y(-120.0));
    path.lineTo(x(760.0), y(-560.0));
    path.lineTo(x(200.0), y(-560.0));
    path.lineTo(x(200.0), y(-120.0));
    path.lineTo(x(120.0), y(-120.0));
    path.close();
    path.moveTo(x(200.0), y(-640.0));
    path.lineTo(x(760.0), y(-640.0));
    path.lineTo(x(760.0), y(-720.0));
    path.lineTo(x(200.0), y(-720.0));
    path.lineTo(x(200.0), y(-640.0));
    path.close();
    path.moveTo(x(440.0), y(-120.0));
    path.lineTo(x(440.0), y(-240.0));
    path.lineTo(x(320.0), y(-240.0));
    path.lineTo(x(320.0), y(-320.0));
    path.lineTo(x(640.0), y(-320.0));
    path.lineTo(x(640.0), y(-240.0));
    path.lineTo(x(520.0), y(-240.0));
    path.lineTo(x(520.0), y(-120.0));
    path.lineTo(x(440.0), y(-120.0));
    path.close();
    path.moveTo(x(200.0), y(-640.0));
    path.lineTo(x(200.0), y(-720.0));
    path.lineTo(x(200.0), y(-640.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
