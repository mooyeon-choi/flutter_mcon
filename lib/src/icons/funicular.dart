import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated funicular icon from Google Material Icons
class MconFunicular extends MconBase {
  const MconFunicular({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconFunicular> createState() => _MconFunicularState();
}

class _MconFunicularState extends MconBaseState<MconFunicular> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconFunicularPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconFunicularPainter extends MconPainter {
  _MconFunicularPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(40.0), y(-40.0));
    path.lineTo(x(40.0), y(-120.0));
    path.lineTo(x(240.0), y(-175.0));
    path.lineTo(x(240.0), y(-280.0));
    path.lineTo(x(120.0), y(-280.0));
    path.lineTo(x(120.0), y(-720.0));
    path.lineTo(x(80.0), y(-720.0));
    path.lineTo(x(80.0), y(-800.0));
    path.lineTo(x(200.0), y(-800.0));
    path.lineTo(x(200.0), y(-880.0));
    path.lineTo(x(760.0), y(-880.0));
    path.lineTo(x(760.0), y(-800.0));
    path.lineTo(x(880.0), y(-800.0));
    path.lineTo(x(880.0), y(-720.0));
    path.lineTo(x(840.0), y(-720.0));
    path.lineTo(x(840.0), y(-360.0));
    path.lineTo(x(720.0), y(-360.0));
    path.lineTo(x(720.0), y(-305.0));
    path.lineTo(x(920.0), y(-360.0));
    path.lineTo(x(920.0), y(-280.0));
    path.lineTo(x(40.0), y(-40.0));
    path.close();
    path.moveTo(x(360.0), y(-207.0));
    path.lineTo(x(600.0), y(-273.0));
    path.lineTo(x(600.0), y(-360.0));
    path.lineTo(x(520.0), y(-360.0));
    path.lineTo(x(520.0), y(-280.0));
    path.lineTo(x(360.0), y(-280.0));
    path.lineTo(x(360.0), y(-207.0));
    path.close();
    path.moveTo(x(200.0), y(-360.0));
    path.lineTo(x(440.0), y(-360.0));
    path.lineTo(x(440.0), y(-400.0));
    path.lineTo(x(200.0), y(-400.0));
    path.lineTo(x(200.0), y(-360.0));
    path.close();
    path.moveTo(x(520.0), y(-440.0));
    path.lineTo(x(760.0), y(-440.0));
    path.lineTo(x(760.0), y(-480.0));
    path.lineTo(x(520.0), y(-480.0));
    path.lineTo(x(520.0), y(-440.0));
    path.close();
    path.moveTo(x(200.0), y(-480.0));
    path.lineTo(x(440.0), y(-480.0));
    path.lineTo(x(440.0), y(-720.0));
    path.lineTo(x(200.0), y(-720.0));
    path.lineTo(x(200.0), y(-480.0));
    path.close();
    path.moveTo(x(520.0), y(-560.0));
    path.lineTo(x(760.0), y(-560.0));
    path.lineTo(x(760.0), y(-720.0));
    path.lineTo(x(520.0), y(-720.0));
    path.lineTo(x(520.0), y(-560.0));
    path.close();
    path.moveTo(x(440.0), y(-360.0));
    path.lineTo(x(440.0), y(-400.0));
    path.lineTo(x(440.0), y(-360.0));
    path.close();
    path.moveTo(x(520.0), y(-440.0));
    path.lineTo(x(520.0), y(-480.0));
    path.lineTo(x(520.0), y(-440.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
