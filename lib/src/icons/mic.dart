...
/// Animated mic icon from Google Material Icons
class MconMic extends MconBase {
  const MconMic({
    super.key,
    super.size,
    super.color,
  ...
  @override
  MconBaseState<MconMic> createState() => _MconMicState();
}

class _MconMicState extends MconBaseState<MconMic> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconMicPainter(
      animation: animation,
...
class _MconMicPainter extends MconPainter {
  _MconMicPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.quadraticBezierTo(x(760.0), y(-415.0), x(692.0), y(-336.0));
    path.quadraticBezierTo(x(624.0), y(-257.0), x(520.0), y(-243.0));
