import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated file_present icon from Google Material Icons
class MconFilePresent extends MconBase {
  const MconFilePresent({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconFilePresent> createState() => _MconFilePresentState();
}

class _MconFilePresentState extends MconBaseState<MconFilePresent> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconFilePresentPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconFilePresentPainter extends MconPainter {
  _MconFilePresentPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(240.0), y(-80.0));
    path.quadraticBezierTo(x(207.0), y(-80.0), x(183.5), y(-103.5));
    path.quadraticBezierTo(x(160.0), y(-127.0), x(160.0), y(-160.0));
    path.lineTo(x(160.0), y(-800.0));
    path.quadraticBezierTo(x(160.0), y(-833.0), x(183.5), y(-856.5));
    path.quadraticBezierTo(x(207.0), y(-880.0), x(240.0), y(-880.0));
    path.lineTo(x(600.0), y(-880.0));
    path.lineTo(x(800.0), y(-680.0));
    path.lineTo(x(800.0), y(-160.0));
    path.quadraticBezierTo(x(800.0), y(-127.0), x(776.5), y(-103.5));
    path.quadraticBezierTo(x(753.0), y(-80.0), x(720.0), y(-80.0));
    path.lineTo(x(240.0), y(-80.0));
    path.close();
    path.moveTo(x(240.0), y(-160.0));
    path.lineTo(x(720.0), y(-160.0));
    path.lineTo(x(720.0), y(-640.0));
    path.lineTo(x(560.0), y(-640.0));
    path.lineTo(x(560.0), y(-800.0));
    path.lineTo(x(240.0), y(-800.0));
    path.lineTo(x(240.0), y(-160.0));
    path.close();
    path.moveTo(x(480.0), y(-200.0));
    path.quadraticBezierTo(x(547.0), y(-200.0), x(593.5), y(-247.0));
    path.quadraticBezierTo(x(640.0), y(-294.0), x(640.0), y(-360.0));
    path.lineTo(x(640.0), y(-520.0));
    path.lineTo(x(560.0), y(-520.0));
    path.lineTo(x(560.0), y(-360.0));
    path.quadraticBezierTo(x(560.0), y(-327.0), x(537.0), y(-303.5));
    path.quadraticBezierTo(x(514.0), y(-280.0), x(480.0), y(-280.0));
    path.quadraticBezierTo(x(447.0), y(-280.0), x(423.5), y(-303.5));
    path.quadraticBezierTo(x(400.0), y(-327.0), x(400.0), y(-360.0));
    path.lineTo(x(400.0), y(-580.0));
    path.quadraticBezierTo(x(400.0), y(-589.0), x(406.0), y(-594.5));
    path.quadraticBezierTo(x(412.0), y(-600.0), x(420.0), y(-600.0));
    path.quadraticBezierTo(x(429.0), y(-600.0), x(434.5), y(-594.5));
    path.quadraticBezierTo(x(440.0), y(-589.0), x(440.0), y(-580.0));
    path.lineTo(x(440.0), y(-360.0));
    path.lineTo(x(520.0), y(-360.0));
    path.lineTo(x(520.0), y(-580.0));
    path.quadraticBezierTo(x(520.0), y(-622.0), x(491.0), y(-651.0));
    path.quadraticBezierTo(x(462.0), y(-680.0), x(420.0), y(-680.0));
    path.quadraticBezierTo(x(378.0), y(-680.0), x(349.0), y(-651.0));
    path.quadraticBezierTo(x(320.0), y(-622.0), x(320.0), y(-580.0));
    path.lineTo(x(320.0), y(-360.0));
    path.quadraticBezierTo(x(320.0), y(-294.0), x(367.0), y(-247.0));
    path.quadraticBezierTo(x(414.0), y(-200.0), x(480.0), y(-200.0));
    path.close();
    path.moveTo(x(240.0), y(-800.0));
    path.lineTo(x(240.0), y(-640.0));
    path.lineTo(x(240.0), y(-800.0));
    path.lineTo(x(240.0), y(-160.0));
    path.lineTo(x(240.0), y(-800.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
