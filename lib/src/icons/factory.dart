import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated factory icon from Google Material Icons
class MconFactory extends MconBase {
  const MconFactory({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconFactory> createState() => _MconFactoryState();
}

class _MconFactoryState extends MconBaseState<MconFactory> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconFactoryPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconFactoryPainter extends MconPainter {
  _MconFactoryPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(80.0), y(-80.0));
    path.lineTo(x(80.0), y(-561.0));
    path.lineTo(x(360.0), y(-680.0));
    path.lineTo(x(360.0), y(-600.0));
    path.lineTo(x(560.0), y(-680.0));
    path.lineTo(x(560.0), y(-560.0));
    path.lineTo(x(880.0), y(-560.0));
    path.lineTo(x(880.0), y(-80.0));
    path.lineTo(x(80.0), y(-80.0));
    path.close();
    path.moveTo(x(160.0), y(-160.0));
    path.lineTo(x(800.0), y(-160.0));
    path.lineTo(x(800.0), y(-480.0));
    path.lineTo(x(480.0), y(-480.0));
    path.lineTo(x(480.0), y(-562.0));
    path.lineTo(x(280.0), y(-482.0));
    path.lineTo(x(280.0), y(-560.0));
    path.lineTo(x(160.0), y(-507.0));
    path.lineTo(x(160.0), y(-160.0));
    path.close();
    path.moveTo(x(440.0), y(-240.0));
    path.lineTo(x(520.0), y(-240.0));
    path.lineTo(x(520.0), y(-400.0));
    path.lineTo(x(440.0), y(-400.0));
    path.lineTo(x(440.0), y(-240.0));
    path.close();
    path.moveTo(x(280.0), y(-240.0));
    path.lineTo(x(360.0), y(-240.0));
    path.lineTo(x(360.0), y(-400.0));
    path.lineTo(x(280.0), y(-400.0));
    path.lineTo(x(280.0), y(-240.0));
    path.close();
    path.moveTo(x(600.0), y(-240.0));
    path.lineTo(x(680.0), y(-240.0));
    path.lineTo(x(680.0), y(-400.0));
    path.lineTo(x(600.0), y(-400.0));
    path.lineTo(x(600.0), y(-240.0));
    path.close();
    path.moveTo(x(880.0), y(-560.0));
    path.lineTo(x(680.0), y(-560.0));
    path.lineTo(x(720.0), y(-880.0));
    path.lineTo(x(840.0), y(-880.0));
    path.lineTo(x(880.0), y(-560.0));
    path.close();
    path.moveTo(x(160.0), y(-160.0));
    path.lineTo(x(800.0), y(-160.0));
    path.lineTo(x(160.0), y(-160.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
