import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated print_error icon from Google Material Icons
class MconPrintError extends MconBase {
  const MconPrintError({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconPrintError> createState() => _MconPrintErrorState();
}

class _MconPrintErrorState extends MconBaseState<MconPrintError> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconPrintErrorPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconPrintErrorPainter extends MconPainter {
  _MconPrintErrorPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(240.0), y(-120.0));
    path.lineTo(x(240.0), y(-280.0));
    path.lineTo(x(80.0), y(-280.0));
    path.lineTo(x(80.0), y(-520.0));
    path.quadraticBezierTo(x(80.0), y(-571.0), x(115.0), y(-605.5));
    path.quadraticBezierTo(x(150.0), y(-640.0), x(200.0), y(-640.0));
    path.lineTo(x(760.0), y(-640.0));
    path.quadraticBezierTo(x(800.0), y(-640.0), x(830.5), y(-617.5));
    path.quadraticBezierTo(x(861.0), y(-595.0), x(873.0), y(-560.0));
    path.lineTo(x(200.0), y(-560.0));
    path.quadraticBezierTo(x(183.0), y(-560.0), x(171.5), y(-548.5));
    path.quadraticBezierTo(x(160.0), y(-537.0), x(160.0), y(-520.0));
    path.lineTo(x(160.0), y(-360.0));
    path.lineTo(x(240.0), y(-360.0));
    path.lineTo(x(240.0), y(-440.0));
    path.lineTo(x(640.0), y(-440.0));
    path.lineTo(x(640.0), y(-360.0));
    path.lineTo(x(320.0), y(-360.0));
    path.lineTo(x(320.0), y(-200.0));
    path.lineTo(x(640.0), y(-200.0));
    path.lineTo(x(640.0), y(-120.0));
    path.lineTo(x(240.0), y(-120.0));
    path.close();
    path.moveTo(x(240.0), y(-640.0));
    path.lineTo(x(240.0), y(-840.0));
    path.lineTo(x(720.0), y(-840.0));
    path.lineTo(x(720.0), y(-640.0));
    path.lineTo(x(640.0), y(-640.0));
    path.lineTo(x(640.0), y(-760.0));
    path.lineTo(x(320.0), y(-760.0));
    path.lineTo(x(320.0), y(-640.0));
    path.lineTo(x(240.0), y(-640.0));
    path.close();
    path.moveTo(x(760.0), y(-120.0));
    path.quadraticBezierTo(x(743.0), y(-120.0), x(731.5), y(-131.5));
    path.quadraticBezierTo(x(720.0), y(-143.0), x(720.0), y(-160.0));
    path.quadraticBezierTo(x(720.0), y(-177.0), x(731.5), y(-188.5));
    path.quadraticBezierTo(x(743.0), y(-200.0), x(760.0), y(-200.0));
    path.quadraticBezierTo(x(777.0), y(-200.0), x(788.5), y(-188.5));
    path.quadraticBezierTo(x(800.0), y(-177.0), x(800.0), y(-160.0));
    path.quadraticBezierTo(x(800.0), y(-143.0), x(788.5), y(-131.5));
    path.quadraticBezierTo(x(777.0), y(-120.0), x(760.0), y(-120.0));
    path.close();
    path.moveTo(x(720.0), y(-280.0));
    path.lineTo(x(720.0), y(-480.0));
    path.lineTo(x(800.0), y(-480.0));
    path.lineTo(x(800.0), y(-280.0));
    path.lineTo(x(720.0), y(-280.0));
    path.close();
    path.moveTo(x(160.0), y(-560.0));
    path.lineTo(x(873.0), y(-560.0));
    path.lineTo(x(160.0), y(-560.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
