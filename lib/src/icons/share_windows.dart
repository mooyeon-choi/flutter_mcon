import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated share_windows icon from Google Material Icons
class MconShareWindows extends MconBase {
  const MconShareWindows({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconShareWindows> createState() => _MconShareWindowsState();
}

class _MconShareWindowsState extends MconBaseState<MconShareWindows> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconShareWindowsPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconShareWindowsPainter extends MconPainter {
  _MconShareWindowsPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(280.0), y(-360.0));
    path.lineTo(x(280.0), y(-600.0));
    path.quadraticBezierTo(x(280.0), y(-633.0), x(303.5), y(-656.5));
    path.quadraticBezierTo(x(327.0), y(-680.0), x(360.0), y(-680.0));
    path.lineTo(x(686.0), y(-680.0));
    path.lineTo(x(583.0), y(-783.0));
    path.lineTo(x(640.0), y(-840.0));
    path.lineTo(x(840.0), y(-640.0));
    path.lineTo(x(640.0), y(-440.0));
    path.lineTo(x(583.0), y(-496.0));
    path.lineTo(x(686.0), y(-600.0));
    path.lineTo(x(360.0), y(-600.0));
    path.lineTo(x(360.0), y(-360.0));
    path.lineTo(x(280.0), y(-360.0));
    path.close();
    path.moveTo(x(200.0), y(-120.0));
    path.quadraticBezierTo(x(167.0), y(-120.0), x(143.5), y(-143.5));
    path.quadraticBezierTo(x(120.0), y(-167.0), x(120.0), y(-200.0));
    path.lineTo(x(120.0), y(-800.0));
    path.lineTo(x(200.0), y(-800.0));
    path.lineTo(x(200.0), y(-200.0));
    path.lineTo(x(680.0), y(-200.0));
    path.lineTo(x(680.0), y(-360.0));
    path.lineTo(x(760.0), y(-360.0));
    path.lineTo(x(760.0), y(-200.0));
    path.quadraticBezierTo(x(760.0), y(-167.0), x(736.5), y(-143.5));
    path.quadraticBezierTo(x(713.0), y(-120.0), x(680.0), y(-120.0));
    path.lineTo(x(200.0), y(-120.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
