...
/// Animated alarm_on icon from Google Material Icons
class MconAlarmOn extends MconBase {
  const MconAlarmOn({
    super.key,
    super.size,
    super.color,
  ...
  @override
  MconBaseState<MconAlarmOn> createState() => _MconAlarmOnState();
}

class _MconAlarmOnState extends MconBaseState<MconAlarmOn> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconAlarmOnPainter(
      animation: animation,
...
class _MconAlarmOnPainter extends MconPainter {
  _MconAlarmOnPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.lineTo(x(607.0), y(-581.0));
    path.lineTo(x(438.0), y(-412.0));
    path.lineTo(x(353.0), y(-497.0));
