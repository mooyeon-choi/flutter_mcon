...
/// Animated grading icon from Google Material Icons
class MconGrading extends MconBase {
  const MconGrading({
    super.key,
    super.size,
    super.color,
  ...
  @override
  MconBaseState<MconGrading> createState() => _MconGradingState();
}

class _MconGradingState extends MconBaseState<MconGrading> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconGradingPainter(
      animation: animation,
...
class _MconGradingPainter extends MconPainter {
  _MconGradingPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path();
    path.moveTo(x(657.0), y(-121.0));
    path.lineTo(x(544.0), y(-234.0));
    path.lineTo(x(600.0), y(-290.0));
    path.lineTo(x(657.0), y(-233.0));
    path.lineTo(x(784.0), y(-360.0));
    path.lineTo(x(840.0), y(-304.0));
    path.lineTo(x(657.0), y(-121.0));
    path.close();
