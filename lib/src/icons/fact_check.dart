...
/// Animated fact_check icon from Google Material Icons
class MconFactCheck extends MconBase {
  const MconFactCheck({
    super.key,
    super.size,
    super.color,
  ...
  @override
  MconBaseState<MconFactCheck> createState() => _MconFactCheckState();
}

class _MconFactCheckState extends MconBaseState<MconFactCheck> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconFactCheckPainter(
      animation: animation,
...
class _MconFactCheckPainter extends MconPainter {
  _MconFactCheckPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.close();
    path.moveTo(x(582.0), y(-360.0));
    path.lineTo(x(780.0), y(-558.0));
    path.lineTo(x(723.0), y(-615.0));
    path.lineTo(x(582.0), y(-473.0));
    path.lineTo(x(525.0), y(-530.0));
    path.lineTo(x(469.0), y(-473.0));
    path.lineTo(x(582.0), y(-360.0));
    path.close();
