...
/// Animated alarm icon from Google Material Icons
class MconAlarm extends MconBase {
  const MconAlarm({
    super.key,
    super.size,
    super.color,
  ...
  @override
  MconBaseState<MconAlarm> createState() => _MconAlarmState();
}

class _MconAlarmState extends MconBaseState<MconAlarm> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconAlarmPainter(
      animation: animation,
...
class _MconAlarmPainter extends MconPainter {
  _MconAlarmPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.close();
    path.moveTo(x(592.0), y(-272.0));
    path.lineTo(x(648.0), y(-328.0));
    path.lineTo(x(520.0), y(-456.0));
    path.lineTo(x(520.0), y(-640.0));
    path.lineTo(x(440.0), y(-640.0));
    path.lineTo(x(440.0), y(-424.0));
    path.lineTo(x(592.0), y(-272.0));
    path.close();
