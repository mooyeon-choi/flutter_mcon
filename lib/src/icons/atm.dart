import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated atm icon from Google Material Icons
class MconAtm extends MconBase {
  const MconAtm({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconAtm> createState() => _MconAtmState();
}

class _MconAtmState extends MconBaseState<MconAtm> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconAtmPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconAtmPainter extends MconPainter {
  _MconAtmPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(415.0), y(-360.0));
    path.lineTo(x(415.0), y(-540.0));
    path.lineTo(x(325.0), y(-540.0));
    path.lineTo(x(325.0), y(-600.0));
    path.lineTo(x(565.0), y(-600.0));
    path.lineTo(x(565.0), y(-540.0));
    path.lineTo(x(475.0), y(-540.0));
    path.lineTo(x(475.0), y(-360.0));
    path.lineTo(x(415.0), y(-360.0));
    path.close();
    path.moveTo(x(80.0), y(-360.0));
    path.lineTo(x(80.0), y(-560.0));
    path.quadraticBezierTo(x(80.0), y(-577.0), x(91.5), y(-588.5));
    path.quadraticBezierTo(x(103.0), y(-600.0), x(120.0), y(-600.0));
    path.lineTo(x(240.0), y(-600.0));
    path.quadraticBezierTo(x(257.0), y(-600.0), x(268.5), y(-588.5));
    path.quadraticBezierTo(x(280.0), y(-577.0), x(280.0), y(-560.0));
    path.lineTo(x(280.0), y(-360.0));
    path.lineTo(x(220.0), y(-360.0));
    path.lineTo(x(220.0), y(-420.0));
    path.lineTo(x(140.0), y(-420.0));
    path.lineTo(x(140.0), y(-360.0));
    path.lineTo(x(80.0), y(-360.0));
    path.close();
    path.moveTo(x(140.0), y(-480.0));
    path.lineTo(x(220.0), y(-480.0));
    path.lineTo(x(220.0), y(-540.0));
    path.lineTo(x(140.0), y(-540.0));
    path.lineTo(x(140.0), y(-480.0));
    path.close();
    path.moveTo(x(620.0), y(-360.0));
    path.lineTo(x(620.0), y(-560.0));
    path.quadraticBezierTo(x(620.0), y(-577.0), x(631.5), y(-588.5));
    path.quadraticBezierTo(x(643.0), y(-600.0), x(660.0), y(-600.0));
    path.lineTo(x(840.0), y(-600.0));
    path.quadraticBezierTo(x(857.0), y(-600.0), x(868.5), y(-588.5));
    path.quadraticBezierTo(x(880.0), y(-577.0), x(880.0), y(-560.0));
    path.lineTo(x(880.0), y(-360.0));
    path.lineTo(x(820.0), y(-360.0));
    path.lineTo(x(820.0), y(-540.0));
    path.lineTo(x(780.0), y(-540.0));
    path.lineTo(x(780.0), y(-400.0));
    path.lineTo(x(720.0), y(-400.0));
    path.lineTo(x(720.0), y(-540.0));
    path.lineTo(x(680.0), y(-540.0));
    path.lineTo(x(680.0), y(-360.0));
    path.lineTo(x(620.0), y(-360.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
