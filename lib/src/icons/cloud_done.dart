...
/// Animated cloud_done icon from Google Material Icons
class MconCloudDone extends MconBase {
  const MconCloudDone({
    super.key,
    super.size,
    super.color,
  ...
  @override
  MconBaseState<MconCloudDone> createState() => _MconCloudDoneState();
}

class _MconCloudDoneState extends MconBaseState<MconCloudDone> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconCloudDonePainter(
      animation: animation,
...
class _MconCloudDonePainter extends MconPainter {
  _MconCloudDonePainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.lineTo(x(640.0), y(-506.0));
    path.lineTo(x(582.0), y(-564.0));
    path.lineTo(x(413.0), y(-395.0));
    path.lineTo(x(329.0), y(-479.0));
    path.lineTo(x(272.0), y(-422.0));
