import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated money icon from Google Material Icons
class MconMoney extends MconBase {
  const MconMoney({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconMoney> createState() => _MconMoneyState();
}

class _MconMoneyState extends MconBaseState<MconMoney> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconMoneyPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconMoneyPainter extends MconPainter {
  _MconMoneyPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(600.0), y(-320.0));
    path.lineTo(x(720.0), y(-320.0));
    path.quadraticBezierTo(x(737.0), y(-320.0), x(748.5), y(-331.5));
    path.quadraticBezierTo(x(760.0), y(-343.0), x(760.0), y(-360.0));
    path.lineTo(x(760.0), y(-600.0));
    path.quadraticBezierTo(x(760.0), y(-617.0), x(748.5), y(-628.5));
    path.quadraticBezierTo(x(737.0), y(-640.0), x(720.0), y(-640.0));
    path.lineTo(x(600.0), y(-640.0));
    path.quadraticBezierTo(x(583.0), y(-640.0), x(571.5), y(-628.5));
    path.quadraticBezierTo(x(560.0), y(-617.0), x(560.0), y(-600.0));
    path.lineTo(x(560.0), y(-360.0));
    path.quadraticBezierTo(x(560.0), y(-343.0), x(571.5), y(-331.5));
    path.quadraticBezierTo(x(583.0), y(-320.0), x(600.0), y(-320.0));
    path.close();
    path.moveTo(x(640.0), y(-400.0));
    path.lineTo(x(640.0), y(-560.0));
    path.lineTo(x(680.0), y(-560.0));
    path.lineTo(x(680.0), y(-400.0));
    path.lineTo(x(640.0), y(-400.0));
    path.close();
    path.moveTo(x(360.0), y(-320.0));
    path.lineTo(x(480.0), y(-320.0));
    path.quadraticBezierTo(x(497.0), y(-320.0), x(508.5), y(-331.5));
    path.quadraticBezierTo(x(520.0), y(-343.0), x(520.0), y(-360.0));
    path.lineTo(x(520.0), y(-600.0));
    path.quadraticBezierTo(x(520.0), y(-617.0), x(508.5), y(-628.5));
    path.quadraticBezierTo(x(497.0), y(-640.0), x(480.0), y(-640.0));
    path.lineTo(x(360.0), y(-640.0));
    path.quadraticBezierTo(x(343.0), y(-640.0), x(331.5), y(-628.5));
    path.quadraticBezierTo(x(320.0), y(-617.0), x(320.0), y(-600.0));
    path.lineTo(x(320.0), y(-360.0));
    path.quadraticBezierTo(x(320.0), y(-343.0), x(331.5), y(-331.5));
    path.quadraticBezierTo(x(343.0), y(-320.0), x(360.0), y(-320.0));
    path.close();
    path.moveTo(x(400.0), y(-400.0));
    path.lineTo(x(400.0), y(-560.0));
    path.lineTo(x(440.0), y(-560.0));
    path.lineTo(x(440.0), y(-400.0));
    path.lineTo(x(400.0), y(-400.0));
    path.close();
    path.moveTo(x(200.0), y(-320.0));
    path.lineTo(x(280.0), y(-320.0));
    path.lineTo(x(280.0), y(-640.0));
    path.lineTo(x(200.0), y(-640.0));
    path.lineTo(x(200.0), y(-320.0));
    path.close();
    path.moveTo(x(80.0), y(-160.0));
    path.lineTo(x(80.0), y(-800.0));
    path.lineTo(x(880.0), y(-800.0));
    path.lineTo(x(880.0), y(-160.0));
    path.lineTo(x(80.0), y(-160.0));
    path.close();
    path.moveTo(x(160.0), y(-720.0));
    path.lineTo(x(160.0), y(-240.0));
    path.lineTo(x(160.0), y(-720.0));
    path.close();
    path.moveTo(x(160.0), y(-240.0));
    path.lineTo(x(800.0), y(-240.0));
    path.lineTo(x(800.0), y(-720.0));
    path.lineTo(x(160.0), y(-720.0));
    path.lineTo(x(160.0), y(-240.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
