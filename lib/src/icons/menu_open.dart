import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated menu_open icon from Google Material Icons
class MconMenuOpen extends MconBase {
  const MconMenuOpen({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconMenuOpen> createState() => _MconMenuOpenState();
}

class _MconMenuOpenState extends MconBaseState<MconMenuOpen> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconMenuOpenPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconMenuOpenPainter extends MconPainter {
  _MconMenuOpenPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(120.0), y(-240.0));
    path.lineTo(x(120.0), y(-320.0));
    path.lineTo(x(640.0), y(-320.0));
    path.lineTo(x(640.0), y(-240.0));
    path.lineTo(x(120.0), y(-240.0));
    path.close();
    path.moveTo(x(784.0), y(-280.0));
    path.lineTo(x(584.0), y(-480.0));
    path.lineTo(x(784.0), y(-680.0));
    path.lineTo(x(840.0), y(-624.0));
    path.lineTo(x(696.0), y(-480.0));
    path.lineTo(x(840.0), y(-336.0));
    path.lineTo(x(784.0), y(-280.0));
    path.close();
    path.moveTo(x(120.0), y(-440.0));
    path.lineTo(x(120.0), y(-520.0));
    path.lineTo(x(520.0), y(-520.0));
    path.lineTo(x(520.0), y(-440.0));
    path.lineTo(x(120.0), y(-440.0));
    path.close();
    path.moveTo(x(120.0), y(-640.0));
    path.lineTo(x(120.0), y(-720.0));
    path.lineTo(x(640.0), y(-720.0));
    path.lineTo(x(640.0), y(-640.0));
    path.lineTo(x(120.0), y(-640.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
