import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated wb_shade icon from Google Material Icons
class MconWbShade extends MconBase {
  const MconWbShade({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconWbShade> createState() => _MconWbShadeState();
}

class _MconWbShadeState extends MconBaseState<MconWbShade> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconWbShadePainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconWbShadePainter extends MconPainter {
  _MconWbShadePainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(780.0), y(-160.0));
    path.lineTo(x(560.0), y(-380.0));
    path.lineTo(x(560.0), y(-480.0));
    path.lineTo(x(880.0), y(-160.0));
    path.lineTo(x(780.0), y(-160.0));
    path.close();
    path.moveTo(x(560.0), y(-160.0));
    path.lineTo(x(560.0), y(-280.0));
    path.lineTo(x(680.0), y(-160.0));
    path.lineTo(x(560.0), y(-160.0));
    path.close();
    path.moveTo(x(160.0), y(-160.0));
    path.lineTo(x(160.0), y(-560.0));
    path.lineTo(x(80.0), y(-560.0));
    path.lineTo(x(320.0), y(-800.0));
    path.lineTo(x(560.0), y(-560.0));
    path.lineTo(x(480.0), y(-560.0));
    path.lineTo(x(480.0), y(-160.0));
    path.lineTo(x(160.0), y(-160.0));
    path.close();
    path.moveTo(x(280.0), y(-400.0));
    path.lineTo(x(360.0), y(-400.0));
    path.lineTo(x(360.0), y(-560.0));
    path.lineTo(x(280.0), y(-560.0));
    path.lineTo(x(280.0), y(-400.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
