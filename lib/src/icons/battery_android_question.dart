...
/// Animated battery_android_question icon from Google Material Icons
class MconBatteryAndroidQuestion extends MconBase {
  const MconBatteryAndroidQuestion({
    super.key,
    super.size,
    super.color,
    super.duration,
  ...
  @override
  MconBaseState<MconBatteryAndroidQuestion> createState() => _MconBatteryAndroidQuestionState();
}

class _MconBatteryAndroidQuestionState extends MconBaseState<MconBatteryAndroidQuestion> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconBatteryAndroidQuestionPainter(
      animation: animation,
...
class _MconBatteryAndroidQuestionPainter extends MconPainter {
  _MconBatteryAndroidQuestionPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.quadraticBezierTo(x(682.0), y(-295.0), x(694.5), y(-274.5));
    path.quadraticBezierTo(x(707.0), y(-254.0), x(726.0), y(-240.0));
    path.close();
    ...
    path.close();
    path.moveTo(x(772.0), y(-422.0));
    path.lineTo(x(829.0), y(-422.0));
    path.lineTo(x(829.0), y(-443.0));
    ...
    path.quadraticBezierTo(x(772.0), y(-437.0), x(772.0), y(-422.0));
    path.close();
