import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated stacked_inbox icon from Google Material Icons
class MconStackedInbox extends MconBase {
  const MconStackedInbox({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconStackedInbox> createState() => _MconStackedInboxState();
}

class _MconStackedInboxState extends MconBaseState<MconStackedInbox> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconStackedInboxPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconStackedInboxPainter extends MconPainter {
  _MconStackedInboxPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(280.0), y(-280.0));
    path.quadraticBezierTo(x(247.0), y(-280.0), x(223.5), y(-303.5));
    path.quadraticBezierTo(x(200.0), y(-327.0), x(200.0), y(-360.0));
    path.lineTo(x(200.0), y(-760.0));
    path.quadraticBezierTo(x(200.0), y(-793.0), x(223.5), y(-816.5));
    path.quadraticBezierTo(x(247.0), y(-840.0), x(280.0), y(-840.0));
    path.lineTo(x(840.0), y(-840.0));
    path.quadraticBezierTo(x(873.0), y(-840.0), x(896.5), y(-816.5));
    path.quadraticBezierTo(x(920.0), y(-793.0), x(920.0), y(-760.0));
    path.lineTo(x(920.0), y(-360.0));
    path.quadraticBezierTo(x(920.0), y(-327.0), x(896.5), y(-303.5));
    path.quadraticBezierTo(x(873.0), y(-280.0), x(840.0), y(-280.0));
    path.lineTo(x(280.0), y(-280.0));
    path.close();
    path.moveTo(x(280.0), y(-480.0));
    path.lineTo(x(280.0), y(-360.0));
    path.lineTo(x(840.0), y(-360.0));
    path.lineTo(x(840.0), y(-480.0));
    path.lineTo(x(698.0), y(-480.0));
    path.quadraticBezierTo(x(677.0), y(-444.0), x(641.0), y(-422.0));
    path.quadraticBezierTo(x(605.0), y(-400.0), x(560.0), y(-400.0));
    path.quadraticBezierTo(x(515.0), y(-400.0), x(479.0), y(-422.0));
    path.quadraticBezierTo(x(443.0), y(-444.0), x(422.0), y(-480.0));
    path.lineTo(x(280.0), y(-480.0));
    path.close();
    path.moveTo(x(560.0), y(-480.0));
    path.quadraticBezierTo(x(593.0), y(-480.0), x(616.5), y(-503.5));
    path.quadraticBezierTo(x(640.0), y(-527.0), x(640.0), y(-560.0));
    path.lineTo(x(840.0), y(-560.0));
    path.lineTo(x(840.0), y(-760.0));
    path.lineTo(x(280.0), y(-760.0));
    path.lineTo(x(280.0), y(-560.0));
    path.lineTo(x(480.0), y(-560.0));
    path.quadraticBezierTo(x(480.0), y(-527.0), x(503.5), y(-503.5));
    path.quadraticBezierTo(x(527.0), y(-480.0), x(560.0), y(-480.0));
    path.close();
    path.moveTo(x(760.0), y(-120.0));
    path.lineTo(x(120.0), y(-120.0));
    path.quadraticBezierTo(x(87.0), y(-120.0), x(63.5), y(-143.5));
    path.quadraticBezierTo(x(40.0), y(-167.0), x(40.0), y(-200.0));
    path.lineTo(x(40.0), y(-680.0));
    path.lineTo(x(120.0), y(-680.0));
    path.lineTo(x(120.0), y(-200.0));
    path.lineTo(x(760.0), y(-200.0));
    path.lineTo(x(760.0), y(-120.0));
    path.close();
    path.moveTo(x(280.0), y(-360.0));
    path.lineTo(x(840.0), y(-360.0));
    path.lineTo(x(280.0), y(-360.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
