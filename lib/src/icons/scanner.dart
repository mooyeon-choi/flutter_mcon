import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated scanner icon from Google Material Icons
class MconScanner extends MconBase {
  const MconScanner({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconScanner> createState() => _MconScannerState();
}

class _MconScannerState extends MconBaseState<MconScanner> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconScannerPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconScannerPainter extends MconPainter {
  _MconScannerPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(704.0), y(-480.0));
    path.lineTo(x(140.0), y(-684.0));
    path.lineTo(x(168.0), y(-760.0));
    path.lineTo(x(792.0), y(-532.0));
    path.quadraticBezierTo(x(812.0), y(-524.0), x(826.0), y(-504.0));
    path.quadraticBezierTo(x(840.0), y(-484.0), x(840.0), y(-460.0));
    path.lineTo(x(840.0), y(-240.0));
    path.quadraticBezierTo(x(840.0), y(-207.0), x(816.5), y(-183.5));
    path.quadraticBezierTo(x(793.0), y(-160.0), x(760.0), y(-160.0));
    path.lineTo(x(200.0), y(-160.0));
    path.quadraticBezierTo(x(167.0), y(-160.0), x(143.5), y(-183.5));
    path.quadraticBezierTo(x(120.0), y(-207.0), x(120.0), y(-240.0));
    path.lineTo(x(120.0), y(-400.0));
    path.quadraticBezierTo(x(120.0), y(-433.0), x(143.5), y(-456.5));
    path.quadraticBezierTo(x(167.0), y(-480.0), x(200.0), y(-480.0));
    path.lineTo(x(704.0), y(-480.0));
    path.close();
    path.moveTo(x(760.0), y(-240.0));
    path.lineTo(x(760.0), y(-400.0));
    path.lineTo(x(200.0), y(-400.0));
    path.lineTo(x(200.0), y(-240.0));
    path.lineTo(x(760.0), y(-240.0));
    path.close();
    path.moveTo(x(400.0), y(-280.0));
    path.lineTo(x(720.0), y(-280.0));
    path.lineTo(x(720.0), y(-360.0));
    path.lineTo(x(400.0), y(-360.0));
    path.lineTo(x(400.0), y(-280.0));
    path.close();
    path.moveTo(x(280.0), y(-280.0));
    path.quadraticBezierTo(x(297.0), y(-280.0), x(308.5), y(-291.5));
    path.quadraticBezierTo(x(320.0), y(-303.0), x(320.0), y(-320.0));
    path.quadraticBezierTo(x(320.0), y(-337.0), x(308.5), y(-348.5));
    path.quadraticBezierTo(x(297.0), y(-360.0), x(280.0), y(-360.0));
    path.quadraticBezierTo(x(263.0), y(-360.0), x(251.5), y(-348.5));
    path.quadraticBezierTo(x(240.0), y(-337.0), x(240.0), y(-320.0));
    path.quadraticBezierTo(x(240.0), y(-303.0), x(251.5), y(-291.5));
    path.quadraticBezierTo(x(263.0), y(-280.0), x(280.0), y(-280.0));
    path.close();
    path.moveTo(x(200.0), y(-240.0));
    path.lineTo(x(200.0), y(-400.0));
    path.lineTo(x(200.0), y(-240.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
