import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated usb icon from Google Material Icons
class MconUsb extends MconBase {
  const MconUsb({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconUsb> createState() => _MconUsbState();
}

class _MconUsbState extends MconBaseState<MconUsb> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconUsbPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconUsbPainter extends MconPainter {
  _MconUsbPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(480.0), y(-80.0));
    path.quadraticBezierTo(x(447.0), y(-80.0), x(423.5), y(-103.5));
    path.quadraticBezierTo(x(400.0), y(-127.0), x(400.0), y(-160.0));
    path.quadraticBezierTo(x(400.0), y(-181.0), x(411.0), y(-199.0));
    path.quadraticBezierTo(x(422.0), y(-217.0), x(440.0), y(-228.0));
    path.lineTo(x(440.0), y(-320.0));
    path.lineTo(x(320.0), y(-320.0));
    path.quadraticBezierTo(x(287.0), y(-320.0), x(263.5), y(-343.5));
    path.quadraticBezierTo(x(240.0), y(-367.0), x(240.0), y(-400.0));
    path.lineTo(x(240.0), y(-492.0));
    path.quadraticBezierTo(x(222.0), y(-501.0), x(211.0), y(-519.0));
    path.quadraticBezierTo(x(200.0), y(-537.0), x(200.0), y(-560.0));
    path.quadraticBezierTo(x(200.0), y(-593.0), x(223.5), y(-616.5));
    path.quadraticBezierTo(x(247.0), y(-640.0), x(280.0), y(-640.0));
    path.quadraticBezierTo(x(313.0), y(-640.0), x(336.5), y(-616.5));
    path.quadraticBezierTo(x(360.0), y(-593.0), x(360.0), y(-560.0));
    path.quadraticBezierTo(x(360.0), y(-537.0), x(349.0), y(-520.0));
    path.quadraticBezierTo(x(338.0), y(-503.0), x(320.0), y(-492.0));
    path.lineTo(x(320.0), y(-400.0));
    path.lineTo(x(440.0), y(-400.0));
    path.lineTo(x(440.0), y(-720.0));
    path.lineTo(x(360.0), y(-720.0));
    path.lineTo(x(480.0), y(-880.0));
    path.lineTo(x(600.0), y(-720.0));
    path.lineTo(x(520.0), y(-720.0));
    path.lineTo(x(520.0), y(-400.0));
    path.lineTo(x(640.0), y(-400.0));
    path.lineTo(x(640.0), y(-480.0));
    path.lineTo(x(600.0), y(-480.0));
    path.lineTo(x(600.0), y(-640.0));
    path.lineTo(x(760.0), y(-640.0));
    path.lineTo(x(760.0), y(-480.0));
    path.lineTo(x(720.0), y(-480.0));
    path.lineTo(x(720.0), y(-400.0));
    path.quadraticBezierTo(x(720.0), y(-367.0), x(696.5), y(-343.5));
    path.quadraticBezierTo(x(673.0), y(-320.0), x(640.0), y(-320.0));
    path.lineTo(x(520.0), y(-320.0));
    path.lineTo(x(520.0), y(-228.0));
    path.quadraticBezierTo(x(539.0), y(-218.0), x(549.5), y(-200.0));
    path.quadraticBezierTo(x(560.0), y(-182.0), x(560.0), y(-160.0));
    path.quadraticBezierTo(x(560.0), y(-127.0), x(536.5), y(-103.5));
    path.quadraticBezierTo(x(513.0), y(-80.0), x(480.0), y(-80.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
