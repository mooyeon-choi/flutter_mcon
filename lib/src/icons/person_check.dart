...
/// Animated person_check icon from Google Material Icons
class MconPersonCheck extends MconBase {
  const MconPersonCheck({
    super.key,
    super.size,
    super.color,
    super.duration,
  ...
  @override
  MconBaseState<MconPersonCheck> createState() => _MconPersonCheckState();
}

class _MconPersonCheckState extends MconBaseState<MconPersonCheck> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconPersonCheckPainter(
      animation: animation,
...
class _MconPersonCheckPainter extends MconPainter {
  _MconPersonCheckPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path();
    path.moveTo(x(702.0), y(-480.0));
    path.lineTo(x(560.0), y(-622.0));
    path.lineTo(x(617.0), y(-678.0));
    path.lineTo(x(702.0), y(-593.0));
    path.lineTo(x(872.0), y(-763.0));
    path.lineTo(x(928.0), y(-706.0));
    path.lineTo(x(702.0), y(-480.0));
    path.close();
