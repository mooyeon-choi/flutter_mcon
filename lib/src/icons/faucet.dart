import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated faucet icon from Google Material Icons
class MconFaucet extends MconBase {
  const MconFaucet({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconFaucet> createState() => _MconFaucetState();
}

class _MconFaucetState extends MconBaseState<MconFaucet> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconFaucetPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconFaucetPainter extends MconPainter {
  _MconFaucetPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(80.0), y(-360.0));
    path.lineTo(x(80.0), y(-440.0));
    path.lineTo(x(280.0), y(-440.0));
    path.lineTo(x(280.0), y(-480.0));
    path.lineTo(x(200.0), y(-480.0));
    path.quadraticBezierTo(x(183.0), y(-480.0), x(171.5), y(-491.5));
    path.quadraticBezierTo(x(160.0), y(-503.0), x(160.0), y(-520.0));
    path.quadraticBezierTo(x(160.0), y(-537.0), x(171.5), y(-548.5));
    path.quadraticBezierTo(x(183.0), y(-560.0), x(200.0), y(-560.0));
    path.lineTo(x(280.0), y(-560.0));
    path.quadraticBezierTo(x(313.0), y(-560.0), x(336.5), y(-536.5));
    path.quadraticBezierTo(x(360.0), y(-513.0), x(360.0), y(-480.0));
    path.lineTo(x(360.0), y(-440.0));
    path.lineTo(x(440.0), y(-440.0));
    path.lineTo(x(440.0), y(-716.0));
    path.quadraticBezierTo(x(440.0), y(-785.0), x(488.0), y(-832.5));
    path.quadraticBezierTo(x(536.0), y(-880.0), x(605.0), y(-880.0));
    path.quadraticBezierTo(x(651.0), y(-880.0), x(690.0), y(-856.0));
    path.quadraticBezierTo(x(729.0), y(-832.0), x(750.0), y(-790.0));
    path.lineTo(x(796.0), y(-698.0));
    path.lineTo(x(724.0), y(-662.0));
    path.lineTo(x(678.0), y(-754.0));
    path.quadraticBezierTo(x(667.0), y(-775.0), x(647.0), y(-787.5));
    path.quadraticBezierTo(x(627.0), y(-800.0), x(604.0), y(-800.0));
    path.quadraticBezierTo(x(569.0), y(-800.0), x(544.5), y(-775.5));
    path.quadraticBezierTo(x(520.0), y(-751.0), x(520.0), y(-716.0));
    path.lineTo(x(520.0), y(-440.0));
    path.lineTo(x(600.0), y(-440.0));
    path.lineTo(x(600.0), y(-480.0));
    path.quadraticBezierTo(x(600.0), y(-513.0), x(623.5), y(-536.5));
    path.quadraticBezierTo(x(647.0), y(-560.0), x(680.0), y(-560.0));
    path.lineTo(x(760.0), y(-560.0));
    path.quadraticBezierTo(x(777.0), y(-560.0), x(788.5), y(-548.5));
    path.quadraticBezierTo(x(800.0), y(-537.0), x(800.0), y(-520.0));
    path.quadraticBezierTo(x(800.0), y(-503.0), x(788.5), y(-491.5));
    path.quadraticBezierTo(x(777.0), y(-480.0), x(760.0), y(-480.0));
    path.lineTo(x(680.0), y(-480.0));
    path.lineTo(x(680.0), y(-440.0));
    path.lineTo(x(880.0), y(-440.0));
    path.lineTo(x(880.0), y(-360.0));
    path.lineTo(x(80.0), y(-360.0));
    path.close();
    path.moveTo(x(240.0), y(-120.0));
    path.quadraticBezierTo(x(207.0), y(-120.0), x(183.5), y(-143.5));
    path.quadraticBezierTo(x(160.0), y(-167.0), x(160.0), y(-200.0));
    path.lineTo(x(160.0), y(-360.0));
    path.lineTo(x(240.0), y(-360.0));
    path.lineTo(x(240.0), y(-200.0));
    path.lineTo(x(720.0), y(-200.0));
    path.lineTo(x(720.0), y(-360.0));
    path.lineTo(x(800.0), y(-360.0));
    path.lineTo(x(800.0), y(-200.0));
    path.quadraticBezierTo(x(800.0), y(-167.0), x(776.5), y(-143.5));
    path.quadraticBezierTo(x(753.0), y(-120.0), x(720.0), y(-120.0));
    path.lineTo(x(240.0), y(-120.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
