import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated edit_audio icon from Google Material Icons
class MconEditAudio extends MconBase {
  const MconEditAudio({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconEditAudio> createState() => _MconEditAudioState();
}

class _MconEditAudioState extends MconBaseState<MconEditAudio> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconEditAudioPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconEditAudioPainter extends MconPainter {
  _MconEditAudioPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(240.0), y(-80.0));
    path.lineTo(x(80.0), y(-240.0));
    path.lineTo(x(240.0), y(-400.0));
    path.lineTo(x(297.0), y(-344.0));
    path.lineTo(x(233.0), y(-280.0));
    path.lineTo(x(727.0), y(-280.0));
    path.lineTo(x(664.0), y(-344.0));
    path.lineTo(x(720.0), y(-400.0));
    path.lineTo(x(880.0), y(-240.0));
    path.lineTo(x(720.0), y(-80.0));
    path.lineTo(x(663.0), y(-136.0));
    path.lineTo(x(727.0), y(-200.0));
    path.lineTo(x(233.0), y(-200.0));
    path.lineTo(x(296.0), y(-136.0));
    path.lineTo(x(240.0), y(-80.0));
    path.close();
    path.moveTo(x(440.0), y(-440.0));
    path.lineTo(x(440.0), y(-920.0));
    path.lineTo(x(520.0), y(-920.0));
    path.lineTo(x(520.0), y(-440.0));
    path.lineTo(x(440.0), y(-440.0));
    path.close();
    path.moveTo(x(280.0), y(-520.0));
    path.lineTo(x(280.0), y(-840.0));
    path.lineTo(x(360.0), y(-840.0));
    path.lineTo(x(360.0), y(-520.0));
    path.lineTo(x(280.0), y(-520.0));
    path.close();
    path.moveTo(x(600.0), y(-520.0));
    path.lineTo(x(600.0), y(-840.0));
    path.lineTo(x(680.0), y(-840.0));
    path.lineTo(x(680.0), y(-520.0));
    path.lineTo(x(600.0), y(-520.0));
    path.close();
    path.moveTo(x(120.0), y(-620.0));
    path.lineTo(x(120.0), y(-740.0));
    path.lineTo(x(200.0), y(-740.0));
    path.lineTo(x(200.0), y(-620.0));
    path.lineTo(x(120.0), y(-620.0));
    path.close();
    path.moveTo(x(760.0), y(-620.0));
    path.lineTo(x(760.0), y(-740.0));
    path.lineTo(x(840.0), y(-740.0));
    path.lineTo(x(840.0), y(-620.0));
    path.lineTo(x(760.0), y(-620.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
