import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated warning_off icon from Google Material Icons
class MconWarningOff extends MconBase {
  const MconWarningOff({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconWarningOff> createState() => _MconWarningOffState();
}

class _MconWarningOffState extends MconBaseState<MconWarningOff> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconWarningOffPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconWarningOffPainter extends MconPainter {
  _MconWarningOffPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(480.0), y(-240.0));
    path.quadraticBezierTo(x(463.0), y(-240.0), x(451.5), y(-251.5));
    path.quadraticBezierTo(x(440.0), y(-263.0), x(440.0), y(-280.0));
    path.quadraticBezierTo(x(440.0), y(-297.0), x(451.5), y(-308.5));
    path.quadraticBezierTo(x(463.0), y(-320.0), x(480.0), y(-320.0));
    path.quadraticBezierTo(x(497.0), y(-320.0), x(508.5), y(-308.5));
    path.quadraticBezierTo(x(520.0), y(-297.0), x(520.0), y(-280.0));
    path.quadraticBezierTo(x(520.0), y(-263.0), x(508.5), y(-251.5));
    path.quadraticBezierTo(x(497.0), y(-240.0), x(480.0), y(-240.0));
    path.close();
    path.moveTo(x(440.0), y(-360.0));
    path.lineTo(x(440.0), y(-521.0));
    path.lineTo(x(520.0), y(-441.0));
    path.lineTo(x(520.0), y(-360.0));
    path.lineTo(x(440.0), y(-360.0));
    path.close();
    path.moveTo(x(873.0), y(-202.0));
    path.lineTo(x(655.0), y(-419.0));
    path.lineTo(x(480.0), y(-720.0));
    path.lineTo(x(433.0), y(-640.0));
    path.lineTo(x(375.0), y(-698.0));
    path.lineTo(x(480.0), y(-880.0));
    path.lineTo(x(873.0), y(-202.0));
    path.close();
    path.moveTo(x(178.0), y(-200.0));
    path.lineTo(x(647.0), y(-200.0));
    path.lineTo(x(350.0), y(-497.0));
    path.lineTo(x(178.0), y(-200.0));
    path.close();
    path.moveTo(x(819.0), y(-28.0));
    path.lineTo(x(727.0), y(-120.0));
    path.lineTo(x(40.0), y(-120.0));
    path.lineTo(x(292.0), y(-555.0));
    path.lineTo(x(27.0), y(-820.0));
    path.lineTo(x(84.0), y(-877.0));
    path.lineTo(x(876.0), y(-85.0));
    path.lineTo(x(819.0), y(-28.0));
    path.close();
    path.moveTo(x(499.0), y(-348.0));
    path.close();
    path.moveTo(x(544.0), y(-529.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
