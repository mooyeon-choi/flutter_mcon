import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated file_save icon from Google Material Icons
class MconFileSave extends MconBase {
  const MconFileSave({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconFileSave> createState() => _MconFileSaveState();
}

class _MconFileSaveState extends MconBaseState<MconFileSave> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconFileSavePainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconFileSavePainter extends MconPainter {
  _MconFileSavePainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(720.0), y(-120.0));
    path.lineTo(x(880.0), y(-280.0));
    path.lineTo(x(824.0), y(-336.0));
    path.lineTo(x(760.0), y(-272.0));
    path.lineTo(x(760.0), y(-439.0));
    path.lineTo(x(680.0), y(-439.0));
    path.lineTo(x(680.0), y(-272.0));
    path.lineTo(x(616.0), y(-336.0));
    path.lineTo(x(560.0), y(-280.0));
    path.lineTo(x(720.0), y(-120.0));
    path.close();
    path.moveTo(x(560.0), y(0.0));
    path.lineTo(x(560.0), y(-80.0));
    path.lineTo(x(880.0), y(-80.0));
    path.lineTo(x(880.0), y(0.0));
    path.lineTo(x(560.0), y(0.0));
    path.close();
    path.moveTo(x(240.0), y(-160.0));
    path.quadraticBezierTo(x(207.0), y(-160.0), x(183.5), y(-183.5));
    path.quadraticBezierTo(x(160.0), y(-207.0), x(160.0), y(-240.0));
    path.lineTo(x(160.0), y(-800.0));
    path.quadraticBezierTo(x(160.0), y(-833.0), x(183.5), y(-856.5));
    path.quadraticBezierTo(x(207.0), y(-880.0), x(240.0), y(-880.0));
    path.lineTo(x(520.0), y(-880.0));
    path.lineTo(x(760.0), y(-640.0));
    path.lineTo(x(760.0), y(-519.0));
    path.lineTo(x(680.0), y(-519.0));
    path.lineTo(x(680.0), y(-600.0));
    path.lineTo(x(480.0), y(-600.0));
    path.lineTo(x(480.0), y(-800.0));
    path.lineTo(x(240.0), y(-800.0));
    path.lineTo(x(240.0), y(-240.0));
    path.lineTo(x(480.0), y(-240.0));
    path.lineTo(x(480.0), y(-160.0));
    path.lineTo(x(240.0), y(-160.0));
    path.close();
    path.moveTo(x(240.0), y(-240.0));
    path.lineTo(x(240.0), y(-800.0));
    path.lineTo(x(240.0), y(-240.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
