...
/// Animated texture_minus icon from Google Material Icons
class MconTextureMinus extends MconBase {
  const MconTextureMinus({
    super.key,
    super.size,
    super.color,
    super.duration,
  ...
  @override
  MconBaseState<MconTextureMinus> createState() => _MconTextureMinusState();
}

class _MconTextureMinusState extends MconBaseState<MconTextureMinus> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconTextureMinusPainter(
      animation: animation,
...
class _MconTextureMinusPainter extends MconPainter {
  _MconTextureMinusPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.close();
    path.moveTo(x(612.0), y(-360.0));
    path.lineTo(x(840.0), y(-588.0));
    path.lineTo(x(840.0), y(-476.0));
    path.lineTo(x(724.0), y(-360.0));
    path.lineTo(x(612.0), y(-360.0));
    path.close();
