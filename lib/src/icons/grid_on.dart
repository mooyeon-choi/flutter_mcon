...
/// Animated grid_on icon from Google Material Icons
class MconGridOn extends MconBase {
  const MconGridOn({
    super.key,
    super.size,
    super.color,
  ...
  @override
  MconBaseState<MconGridOn> createState() => _MconGridOnState();
}

class _MconGridOnState extends MconBaseState<MconGridOn> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconGridOnPainter(
      animation: animation,
...
class _MconGridOnPainter extends MconPainter {
  _MconGridOnPainter({
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.close();
    path.moveTo(x(413.0), y(-200.0));
    path.lineTo(x(547.0), y(-200.0));
    path.lineTo(x(547.0), y(-333.0));
