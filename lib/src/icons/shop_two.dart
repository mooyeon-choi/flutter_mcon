import 'package:flutter/widgets.dart';
import '../core/mcon_base.dart';

/// Animated shop_two icon from Google Material Icons
class MconShopTwo extends MconBase {
  const MconShopTwo({
    super.key,
    super.size,
    super.color,
    super.duration,
    super.curve,
    super.animationType,
    super.animationDirection,
  });

  @override
  MconBaseState<MconShopTwo> createState() => _MconShopTwoState();
}

class _MconShopTwoState extends MconBaseState<MconShopTwo> {
  @override
  CustomPainter createPainter(Animation<double> animation) {
    return _MconShopTwoPainter(
      animation: animation,
      color: widget.color ?? const Color(0xFF000000),
    );
  }
}

class _MconShopTwoPainter extends MconPainter {
  _MconShopTwoPainter({
    required super.animation,
    required super.color,
  });

  @override
  void paint(Canvas canvas, Size size) {
    final progress = animation.value;
    final scaleX = size.width / 960;
    final scaleY = size.height / 960;

    double x(double coord) => coord * scaleX;
    double y(double coord) => (coord + 960) * scaleY;

    final path = Path();
    path.moveTo(x(480.0), y(-360.0));
    path.lineTo(x(700.0), y(-500.0));
    path.lineTo(x(480.0), y(-640.0));
    path.lineTo(x(480.0), y(-360.0));
    path.close();
    path.moveTo(x(120.0), y(-80.0));
    path.quadraticBezierTo(x(87.0), y(-80.0), x(63.5), y(-103.5));
    path.quadraticBezierTo(x(40.0), y(-127.0), x(40.0), y(-160.0));
    path.lineTo(x(40.0), y(-600.0));
    path.lineTo(x(120.0), y(-600.0));
    path.lineTo(x(120.0), y(-160.0));
    path.lineTo(x(800.0), y(-160.0));
    path.lineTo(x(800.0), y(-80.0));
    path.lineTo(x(120.0), y(-80.0));
    path.close();
    path.moveTo(x(280.0), y(-240.0));
    path.quadraticBezierTo(x(247.0), y(-240.0), x(223.5), y(-263.5));
    path.quadraticBezierTo(x(200.0), y(-287.0), x(200.0), y(-320.0));
    path.lineTo(x(200.0), y(-760.0));
    path.lineTo(x(400.0), y(-760.0));
    path.lineTo(x(400.0), y(-840.0));
    path.quadraticBezierTo(x(400.0), y(-873.0), x(423.5), y(-896.5));
    path.quadraticBezierTo(x(447.0), y(-920.0), x(480.0), y(-920.0));
    path.lineTo(x(640.0), y(-920.0));
    path.quadraticBezierTo(x(673.0), y(-920.0), x(696.5), y(-896.5));
    path.quadraticBezierTo(x(720.0), y(-873.0), x(720.0), y(-840.0));
    path.lineTo(x(720.0), y(-760.0));
    path.lineTo(x(920.0), y(-760.0));
    path.lineTo(x(920.0), y(-320.0));
    path.quadraticBezierTo(x(920.0), y(-287.0), x(896.5), y(-263.5));
    path.quadraticBezierTo(x(873.0), y(-240.0), x(840.0), y(-240.0));
    path.lineTo(x(280.0), y(-240.0));
    path.close();
    path.moveTo(x(280.0), y(-320.0));
    path.lineTo(x(840.0), y(-320.0));
    path.lineTo(x(840.0), y(-680.0));
    path.lineTo(x(280.0), y(-680.0));
    path.lineTo(x(280.0), y(-320.0));
    path.close();
    path.moveTo(x(480.0), y(-760.0));
    path.lineTo(x(640.0), y(-760.0));
    path.lineTo(x(640.0), y(-840.0));
    path.lineTo(x(480.0), y(-840.0));
    path.lineTo(x(480.0), y(-760.0));
    path.close();
    path.moveTo(x(280.0), y(-320.0));
    path.lineTo(x(280.0), y(-680.0));
    path.lineTo(x(280.0), y(-320.0));
    path.close();

    final paint = Paint()
      ..style = PaintingStyle.fill
      ..color = color.withValues(alpha: progress);

    canvas.drawPath(path, paint);
  }
}
